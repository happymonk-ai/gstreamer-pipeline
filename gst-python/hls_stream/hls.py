...
def launchHLSstream(camID, rtsp_URL, path, camera_type):
    ...
    if(camera_type == 'H264'):
        gst_str = 'rtspsrc name=m_rtspsrc_{ID} latency=0 location={rtsp} protocols=tcp drop-on-latency=true ! rtph264depay name=m_depay_{ID} ! mpegtsmux name=m_mux_{ID} ! hlssink name=m_sink_{ID} playlist-root=https://hls.ckdr.co.in/live/stream{ID} playlist-location={file_path}/{ID}.m3u8 location={file_path}/segment.%01d.ts target-duration=10 playlist-length=3 max-files=6'.format(ID = camID, rtsp=rtsp_URL, file_path = path)
    if(camera_type == 'H265'):
        gst_str = 'rtspsrc name=m_rtspsrc_{ID} latency=0 location={rtsp} protocols=tcp drop-on-latency=true ! rtph265depay name=m_depay_{ID} ! mpegtsmux name=m_mux_{ID} ! hlssink name=m_sink_{ID} playlist-root=https://hls.ckdr.co.in/live/stream{ID} playlist-location={file_path}/{ID}.m3u8 location={file_path}/segment.%01d.ts target-duration=10 playlist-length=3 max-files=6'.format(ID = camID, rtsp=rtsp_URL, file_path = path)
