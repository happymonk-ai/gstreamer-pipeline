...
struct pair pairs[2] = {
	{"/home/nivetheni/nats.c/device_stream/video23.mp4", "haKDBkjhadlk", "1"},
	{"/home/nivetheni/nats.c/device_stream/test_video.mp4", "ioalkjNmahnKL", "2"}};
...
static gboolean add_mp4(gchar *device_id, gchar *device_url, gchar *iteration);
...
int main (int argc, char *argv[])
{
    ...
    for (int i = 0; i < 2; i++)
    {
      ...
      args->iter = pairs[i].str3;
      
      if (!add_mp4(args->url, args->deviceid, args->iter)){
...
static gboolean add_mp4(gchar *location, gchar *device_id, gchar *iteration)
{
...
// gst_parse_launch("filesrc location=./gstreamer_tutorials/sample2.mp4 ! decodebin ! videoconvert ! videoscale ! video/x-raw, format=GRAY8, width = 512, height = 512 ! appsink name=sink",NULL);
    ...
    g_signal_connect(data.app_sink, "new-sample", G_CALLBACK(new_sample), iteration);
...
static GstFlowReturn new_sample(GstAppSink *sink, gpointer user_data)
{
    ...
    gchar *frame_iter = user_data;

    printf("The stream has started\n");

    gchar *subject = g_strdup_printf ("device.%s.frame", frame_iter);
