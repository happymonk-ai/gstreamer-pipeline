...
def launchHLSstream(camID, rtsp_URL, path, camera_type):
    ...
    if((camera_type.lower()) == "mp4"):
        gst_str = 'rtspsrc name=m_rtspsrc_{ID} ! decodebin name=m_decode_{ID} ! x264enc name=m_enc_{ID} ! mpegtsmux name=m_mux_{ID} ! hlssink name=m_sink_{ID}'.format(ID = camID)
