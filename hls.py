...
def launchHLSstream(camID, rtsp_URL, path, camera_type):
    ...
    if(camera_type == 'H.264'):
        gst_str = 'rtspsrc name=m_rtspsrc_{ID} ! rtph264depay name=m_depay_{ID} ! h264parse name=m_parse_{ID} ! decodebin name=m_decode_{ID} ! clockoverlay name=m_overlay_{ID} ! videoconvert name=m_videoconvert_{ID} ! videorate name=m_videorate_{ID} ! x264enc name=m_enc_{ID} ! mpegtsmux name=m_mux_{ID} ! hlssink name=m_sink_{ID}'.format(ID = camID)
    if(camera_type == 'H.265'):
        gst_str = 'rtspsrc name=m_rtspsrc_{ID} ! rtph265depay name=m_depay_{ID} ! h265parse name=m_parse_{ID} ! decodebin name=m_decode_{ID} ! clockoverlay name=m_overlay_{ID} ! videoconvert name=m_videoconvert_{ID} ! videorate name=m_videorate_{ID} ! x265enc name=m_enc_{ID} ! mpegtsmux name=m_mux_{ID} ! hlssink name=m_sink_{ID}'.format(ID = camID)
    ...
    source.set_property('timeout', 50)
    source.set_property('tcp-timeout', 10000000)
    ...
    # overlay params
    overlay = pipeline.get_by_name('m_overlay_{ID}'.format(ID = camID))

    # convert params
    convert = pipeline.get_by_name('m_videoconvert_{ID}'.format(ID = camID))
    
    # videorate params
    videorate = pipeline.get_by_name('m_videorate_{ID}'.format(ID = camID))

    #framerate parameters
    framerate_ctr = pipeline.get_by_name('m_videorate_{ID}'.format(ID = camID))
    framerate_ctr.set_property('max-rate', 30/1)
    ...
    if not source or not sink or not pipeline or not depay or not parse or not decode or not convert or not overlay or not videorate or not encode or not mux:
