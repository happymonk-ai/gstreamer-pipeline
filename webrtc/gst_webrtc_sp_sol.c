...
static GstElement *pipe1, *webrtc1, *app_sink;
...
start_pipeline (void)
{
  ...
  pipe1 = gst_parse_launch ("webrtcbin bundle-policy=max-bundle name=sendrecv stun-server=stun://stun.l.google.com:19302 rtspsrc location=rtsp://127.0.0.1:8554//test1 latency=0 ! rtph265depay ! h265parse ! queue ! decodebin ! videoconvert ! queue ! vp8enc deadline=1 ! rtpvp8pay ! queue ! application/x-rtp,media=video,encoding-name=VP8,payload= 96  ! sendrecv.", &error);
...
static gboolean create_pipeline()
{
    ...
    pipe1 = gst_parse_launch ("rtspsrc location=rtsp://127.0.0.1:8554//test1 latency=0 ! rtph265depay ! h265parse ! tee name=t ! queue ! decodebin ! videoconvert ! videoscale ! video/x-raw, format=GRAY8, width = 1024, height = 1024 ! appsink name=sink", &error);
    ...
    app_sink = gst_bin_get_by_name (GST_BIN (pipe1), "sink");
    g_assert_nonnull (app_sink);

    g_object_set(app_sink, "emit-signals", TRUE, NULL);
    g_signal_connect(app_sink, "new-sample", G_CALLBACK(new_sample), app_sink);
