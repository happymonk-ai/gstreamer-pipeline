...
static gboolean hls_function(char *file_path, char *file_url)
{
  ...
  gst_str1 = g_strdup_printf("rtspsrc location=%s ! rtph264depay ! avdec_h264 ! clockoverlay ! videoconvert ! videoscale ! video/x-raw,width=1080, height=1080 ! x264enc bitrate=512 ! video/x-h264,profile=\"high\" ! hlssink2 playlist-root=http://216.48.189.5:8080 playlist-location=%s/playlist.m3u8 location=%s", file_url, file_path, file_path);

  gst_str2 = "/segment.%05d.ts target-duration=15 max-files=10 playlist-length=5";
