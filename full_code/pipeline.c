// gcc pipeline.c -o pipeline -lgstnet-1.0 `pkg-config --cflags --libs gstreamer-1.0 gstreamer-app-1.0 gstreamer-rtsp-server-1.0 json-c libnats`
...
static const char *interpipe_subject = "server.add.stream";
...
const char *stream1 = "mp4_camera_stream";
...
static gboolean hls_server_device(char *file_name, char *file_url, char *file_path)
{
  ...
  gst_str1 = g_strdup_printf("rtspsrc location=%s ! queue ! rtph265depay ! h265parse ! mpegtsmux ! hlssink playlist-root=http://localhost:8060 playlist-location=%s/%s.m3u8 location=%s", file_url, file_path, file_name, file_path);
...
static gboolean hls_server_mp4(char *file_name, char *file_url, char *file_path)
{
    ...
    g_print("HLS FILE NAME: %s\n", file_name);
    g_print("HLS FILE URL: %s\n", file_url);
    g_print("HLS FILE PATH: %s\n", file_path);

    gchar *gst_str1, *gst_str2, *gst_str3;

    gst_str1 = g_strdup_printf("rtspsrc location=%s ! rtph264depay ! avdec_h264 ! clockoverlay ! videoconvert ! videoscale ! video/x-raw,width=1080, height=1080 ! x264enc bitrate=512 ! video/x-h264,profile=\"high\" ! hlssink2 playlist-root=http://localhost:8060 playlist-location=%s/%s.m3u8 location=%s", file_url, file_path, file_name, file_path);
...
static GstFlowReturn new_sample_1(GstAppSink *sink, gpointer user_data)
{
    GstSample *sample;
    GstBuffer *buffer;
    ...
    struct json_object *json_obj_4, *video_id_4, *video_path_4;
    const char *id_4, *path_4, *jstr_4;

    gchar *json_str = user_data;

    g_print("THE JSON STRING OBJECT IS %s\n", json_str);

    json_obj_4 = json_tokener_parse(json_str);
    video_id_4 = find_something(json_obj_4, "video_id");

    id_4 = json_object_to_json_string_ext(video_id_4, JSON_C_TO_STRING_SPACED | JSON_C_TO_STRING_PRETTY);

    gchar *subj1 = g_strdup_printf("stream.%s.frame", id_4);
    ...
    if (sample)
    {
        ...
        {
            json_object_object_add(json_obj_4, "frame_in_bytes", json_object_new_string(info.data));

            jstr_4 = json_object_to_json_string_ext(json_obj_4, JSON_C_TO_STRING_SPACED | JSON_C_TO_STRING_PRETTY);
            printf("%s\n", jstr_4);

            for (count = 0; (s == NATS_OK) && (count < 1); count++)
            {
                s = js_PublishAsync(js, subj1, (const void *)(jstr_4), info.size, NULL);
                // exit (-1);
            }
...
static GstFlowReturn new_sample(GstAppSink *sink, gpointer user_data)
{
    ...
    gint width, height;
    GstClockTime timestamp;
    struct json_object *json_obj_2, *video_id_2, *video_path_2;
    const char *id_2, *path_2, *jstr_2;

    gchar *json_str = user_data;

    g_print("THE JSON STRING OBJECT IS %s\n", json_str);

    json_obj_2 = json_tokener_parse(json_str);
    video_id_2 = find_something(json_obj_2, "video_id");

    id_2 = json_object_to_json_string_ext(video_id_2, JSON_C_TO_STRING_SPACED | JSON_C_TO_STRING_PRETTY);

    gchar *subj1 = g_strdup_printf("stream.%s.frame", id_2);
    ...
    if (sample)
    {
        ...
        if (gst_buffer_map(buffer, &info, (GstMapFlags)(GST_MAP_READ)))
        {
            json_object_object_add(json_obj_2, "frame_in_bytes", json_object_new_string(info.data));

            jstr_2 = json_object_to_json_string_ext(json_obj_2, JSON_C_TO_STRING_SPACED | JSON_C_TO_STRING_PRETTY);
            printf("%s\n", jstr_2);
        }

        char array[30][info.size];

        strcpy(array[i_1], jstr_2);

        if (i_1 == 29)
        {

            for (count = 0; (s == NATS_OK) && (count < 30); count++)
            {
                g_print("DATA: %s\n", array[count]);
                s = js_PublishAsync(js, subj1, (const void *)(array[count]), info.size, NULL);
                // exit (-1);
            }
...
static gboolean add_device(struct json_object *json_obj_3)
{

    struct json_object *video_id_3, *video_path_3;
    const char *jstr_3, *id_3, *path_3;
    char *new_jstr_3, *new_id_3, *new_path_3;

    video_id_3 = find_something(json_obj_3, "video_id");
    video_path_3 = find_something(json_obj_3, "video_path");

    jstr_3 = json_object_to_json_string_ext(json_obj_3, JSON_C_TO_STRING_SPACED | JSON_C_TO_STRING_PRETTY);
    id_3 = json_object_to_json_string_ext(video_id_3, JSON_C_TO_STRING_SPACED | JSON_C_TO_STRING_PRETTY);
    path_3 = json_object_to_json_string_ext(video_path_3, JSON_C_TO_STRING_SPACED | JSON_C_TO_STRING_PRETTY);

    new_jstr_3 = malloc(strlen(jstr_3) + 1);
    new_id_3 = malloc(strlen(id_3) + 1);
    new_path_3 = malloc(strlen(path_3) + 1);

    strcpy(new_jstr_3, jstr_3);
    strcpy(new_id_3, id_3);
    strcpy(new_path_3, path_3);

    new_id_3 = removeChar(new_id_3, '"');
    new_path_3 = removeChar(new_path_3, '\\');
    new_path_3 = removeChar(new_path_3, '"');

    g_print("JSON STRING: %s\n", new_jstr_3);
    g_print("FILE ID: %s\n", new_id_3);
    g_print("FILE PATH: %s\n", new_path_3);
    ...
    g_signal_connect(app_sink, "new-sample", G_CALLBACK(new_sample_1), new_jstr_3);
...
static gboolean add_mp4(struct json_object *json_obj_1)
{
    struct json_object *video_id_1, *video_path_1;
    const char *jstr_1, *id_1, *path_1;
    char *new_jstr_1, *new_id_1, *new_path_1;

    video_id_1 = find_something(json_obj_1, "video_id");
    video_path_1 = find_something(json_obj_1, "video_path");

    jstr_1 = json_object_to_json_string_ext(json_obj_1, JSON_C_TO_STRING_SPACED | JSON_C_TO_STRING_PRETTY);
    id_1 = json_object_to_json_string_ext(video_id_1, JSON_C_TO_STRING_SPACED | JSON_C_TO_STRING_PRETTY);
    path_1 = json_object_to_json_string_ext(video_path_1, JSON_C_TO_STRING_SPACED | JSON_C_TO_STRING_PRETTY);

    new_jstr_1 = malloc(strlen(jstr_1) + 1);
    new_id_1 = malloc(strlen(id_1) + 1);
    new_path_1 = malloc(strlen(path_1) + 1);

    strcpy(new_jstr_1, jstr_1);
    strcpy(new_id_1, id_1);
    strcpy(new_path_1, path_1);

    new_id_1 = removeChar(new_id_1, '"');
    new_path_1 = removeChar(new_path_1, '\\');
    new_path_1 = removeChar(new_path_1, '"');
    ...
    sink_name = g_strdup_printf("sink-%s", new_id_1);

    gst_str = g_strdup_printf("filesrc location=%s ! decodebin name=decode-%s ! videoconvert name=convert-%s ! videoscale name=scale-%s ! video/x-raw, format=GRAY8, width = 150, height = 255 ! appsink name=%s", new_path_1, new_id_1, new_id_1, new_id_1, sink_name);
    ...
    g_object_set(app_sink, "emit-signals", TRUE, NULL);
    g_signal_connect(app_sink, "new-sample", G_CALLBACK(new_sample), new_jstr_1);
...
{
    gchar *server_str_1, *file_path_1;
    ...
    json_object_object_add(json_obj, "stream_url", json_object_new_string(server_str_1));

    if (!add_device(json_obj))
    {
...
static gboolean video_server(char *endpt, char *device_url, char *device_id, struct json_object *json_obj)
{
    ...
    json_object_object_add(json_obj, "video_url", json_object_new_string(server_str));
    ...
    file_path = g_strdup_printf("/home/nivetheni/full_code/video_%s", device_id);
...
onMsg(natsConnection *nc, natsSubscription *sub, natsMsg *msg, void *closure)
{
    ...
    video_id = find_something(jobj, "video_id");
    video_path = find_something(jobj, "video_path");
    ...
    case 'a':
        ...
        if (strcmp(new_type, "stream") == 0)
        {
            ...
            {
                g_printerr("Cannot add the device stream to RTSP Server\n");
            }
        }
        break;
    case 'r':
...
int main(int argc, gchar *argv[])
{
    ...
    // net_clock = gst_net_client_clock_new("net_clock", clock_server, clock_port, 0);
    // if (net_clock == NULL)
    // {
    //     g_print("Failed to create net clock client for %s:%d\n",
    //             clock_server, clock_port);
    //     return 1;
    // }
