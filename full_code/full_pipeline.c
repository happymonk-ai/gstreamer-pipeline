...
static gboolean add_mp4(char *id, char *location, struct json_object *json_obj);
...
{
    gchar *device_id, *file_loc, *json_str_1, *new_id_1;
    struct json_object *jobj_1, *video_id_1;
    const gchar *id_1;

    json_str_1 = data;

    jobj_1 = json_tokener_parse(json_str_1);
    video_id_1 = find_something(jobj_1, "device_id");

    id_1 = json_object_to_json_string(video_id_1);
    ...
    if (!add_mp4(new_id_1, file_loc, jobj_1))
    {
...
static gboolean hls_server_mp4(char *file_name, char *file_url, char *file_path)
{
    ...
    gst_str1 = g_strdup_printf("rtspsrc location=%s ! rtph264depay ! avdec_h264 ! clockoverlay ! videoconvert ! videoscale ! video/x-raw,width=1080, height=1080 ! x264enc bitrate=512 ! video/x-h264,profile=\"high\" ! hlssink2 playlist-root=https://streams.ckdr.co.in/live/stream%s playlist-location=%s/%s.m3u8 location=%s", file_url, file_name, file_path, file_name, file_path);
...
static GstFlowReturn new_sample(GstAppSink *sink, gpointer user_data)
{
    ...
    gchar *subj1 = g_strdup_printf("device.%s.frame", id);
...
static gboolean add_mp4(char *id, char *location, struct json_object *json_obj)
{
    ...
    /*callback function to fetch the frames from sample*/
    g_object_set(app_sink, "emit-signals", TRUE, NULL);
    g_signal_connect(app_sink, "new-sample", G_CALLBACK(new_sample), new_json_str);
    g_signal_connect(app_sink, "eos", G_CALLBACK(eos_sample), new_json_str);
...
static gboolean video_server(char *device_url, char *device_id, struct json_object *json_obj)
{
    gchar *server_str, *file_path, *hls_str;
    // const char *json_str;
    char *endpt;

    endpt = g_strdup_printf("/video%s", device_id);
    ...
    if (!add_mp4(device_id, device_url, json_obj))
    {
        g_printerr("Cannot add stream to Jetstream!\n");
    }
...
int main(int argc, gchar *argv[])
{
    ...
    for (int i = 1; i <= 5; i++)
    {
        ...
        if (!video_server(new_path, new_id, init_json1))
        {
            g_printerr("Cannot add the mp4 stream to RTSP Server\n");
        }
        sleep(5);
