...
static GstFlowReturn new_sample(GstAppSink *sink, gpointer user_data)
{
    ...
    struct json_object *json_obj, *video_id;
    const gchar *id, *path, *jstr;
    ...
    video_id = find_something(json_obj, "video_id");
    ...
    if (sample)
    {
        ...
        if (gst_buffer_map(buffer, &info, (GstMapFlags)(GST_MAP_READ)))
        {
            ...
            // g_print("FRAME DATA: %s\n", info.data);
            // g_print("TIMESTAMP: %lu\n", timestamp);
            json_object_object_add(json_obj, "frame_in_bytes", json_object_new_string(info.data));

            json_object_object_add(json_obj, "frame_timestamp", json_object_new_double(timestamp));

            jstr = json_object_to_json_string_ext(json_obj, JSON_C_TO_STRING_SPACED | JSON_C_TO_STRING_PRETTY);
            // printf("%s\n", jstr);

            s = js_PublishAsync(js, subj1, (const void *)(jstr), info.size, NULL);
...
static gboolean add_mp4(char *id, char *location, char *json_str)
{
    ...
    gst_str = g_strdup_printf("filesrc location=%s ! decodebin name=decode-%s ! videoconvert name=convert-%s ! videoscale name=scale-%s ! video/x-raw, format=GRAY8, width = 124, height = 124 ! appsink name=%s", location, id, id, id, sink_name);
